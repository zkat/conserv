...
(defun socket-connect (socket host port &key (wait t))
  (let ((internal-socket (iolib:make-socket :connect :active :ipv6 nil)))
    (handler-bind ((error (lambda (e)
                            (on-socket-error (socket-driver socket) socket e))))
      (restart-case
          (iolib:connect internal-socket (iolib:lookup-hostname host) :port port :wait wait)
