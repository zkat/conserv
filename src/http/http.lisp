...
(defun parse-headers (server driver data)
  (multiple-value-bind (donep parser rest)
    ...
    (when donep
      ...
      (when (eq :http-1.0 (request-http-version *request*))
        (setf (request-keep-alive-p *request*) nil)))
    (when rest
      (if donep
          (let ((header-info (request-headers-in *request*)))
            (cond ((or
                  ...
                  (t
                   ...
                   (when (string= (request-header header-info :connection) "keep-alive")
                     (setf (request-keep-alive-p *request*) t))
                   (when (string= (request-header header-info :connection) "close")
                     (setf (request-keep-alive-p *request*) nil))
